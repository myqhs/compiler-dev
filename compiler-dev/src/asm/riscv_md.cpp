...
void RiscvDesc::emitPieces(scope::GlobalScope *gscope, Piece *ps,
  ...
  while (NULL != ps) {
    ...
    case Piece::GLOBAL:
      ...
      emit(EMPTY_STR,
           ((std::string)(".global ") + ps->as.globalVar->name).c_str(), NULL);
      emit(EMPTY_STR,
           ((std::string)(".size ") + ps->as.globalVar->name +
            (std::string)(", ") + std::to_string(ps->as.globalVar->size))
               .c_str(),
           NULL);
      emit(ps->as.globalVar->name.c_str(), NULL, NULL);
      if (ps->as.globalVar->value != 0) {
        emit(EMPTY_STR,
             ((std::string)(".word ") + std::to_string(ps->as.globalVar->value))
                 .c_str(),
             NULL);
      } else
        emit(EMPTY_STR,
             ((std::string)(".zero ") + std::to_string(ps->as.globalVar->size))
                 .c_str(),
             NULL);
      break;
...
RiscvInstr *RiscvDesc::prepareSingleChain(BasicBlock *b, FlowGraph *g) {
  ...
  case BasicBlock::BY_RETURN:
    ...
    addInstr(RiscvInstr::MOVE, _reg[RiscvReg::A0], _reg[r0], NULL, 0, EMPTY_STR,
             NULL);
    addInstr(RiscvInstr::MOVE, _reg[RiscvReg::SP], _reg[RiscvReg::FP], NULL, 0,
             EMPTY_STR, NULL);
    addInstr(RiscvInstr::LW, _reg[RiscvReg::RA], _reg[RiscvReg::FP], NULL, -4,
             EMPTY_STR, NULL);
    addInstr(RiscvInstr::LW, _reg[RiscvReg::FP], _reg[RiscvReg::FP], NULL, -8,
             EMPTY_STR, NULL);
...
void RiscvDesc::emitTac(Tac *t) {
  ...
  case Tac::POP:
    addInstr(RiscvInstr::ADDI, _reg[RiscvReg::SP], _reg[RiscvReg::SP], NULL, 4,
             EMPTY_STR, NULL);
    break;
  ...
  case Tac::CALL:
    emitCallTac(t);
    /*{
        addInstr(RiscvInstr::CALL, NULL, NULL, NULL, 0, std::string("_") +
       t->op1.label->str_form, NULL); int r0 = getRegForWrite(t->op0.var, 0, 0,
       t->LiveOut); addInstr(RiscvInstr::MOVE, _reg[r0], _reg[RiscvReg::A0],
       NULL, 0, EMPTY_STR, NULL);
        }
    */
    break;
...
void RiscvDesc::emitCallTac(Tac *t) {
...

  {
    ...
    for (auto temp : *liveness) {
      ...
      addInstr(RiscvInstr::SW, _reg[r1], _reg[RiscvReg::SP], NULL, cnt,
               EMPTY_STR, NULL);
    }
  ...
  int count = 0;
  for (Tac *it = t->prev; it != NULL && it->op_code == Tac::PARAM;
       it = it->prev)
    count += 4;

  if (count > 0) {
    addInstr(RiscvInstr::ADDI, _reg[RiscvReg::SP], _reg[RiscvReg::SP], NULL,
             -count, EMPTY_STR, NULL);
    int cnt = count;
    for (Tac *it = t->prev; it != NULL && it->op_code == Tac::PARAM;
         it = it->prev) {
      cnt -= 4;
  ...
  // printf("%d\n", r0);
  {
    ...
    for (auto temp : *liveness) {
      ...
      addInstr(RiscvInstr::LW, _reg[r1], _reg[RiscvReg::SP], NULL, cnt,
               EMPTY_STR, NULL);
    }
  }
...
void RiscvDesc::emitPushTac(Tac *t) {
  ...
  addInstr(RiscvInstr::ADDI, _reg[RiscvReg::SP], _reg[RiscvReg::SP], NULL, -4,
           EMPTY_STR, NULL);
  addInstr(RiscvInstr::SW, _reg[r1], _reg[RiscvReg::SP], NULL, 0, EMPTY_STR,
           NULL);
}
...
void RiscvDesc::emitLoadImm4Tac(Tac *t) {
  ...
  int r0 = getRegForWrite(t->op0.var, 0, 0, t->LiveOut);
  addInstr(RiscvInstr::LI, _reg[r0], NULL, NULL, t->op1.ival, EMPTY_STR, NULL);
}
...
int RiscvDesc::getRegForRead(Temp v, int avoid1, LiveSet *live) {
  ...
  if (i < 0) {
    ...
    i = lookupReg(NULL);
    if (i < 0) {
      i = selectRegToSpill(avoid1, RiscvReg::ZERO, live); //选择寄存器
      spillReg(i, live);                                  //溢出
    }
...
void RiscvDesc::spillReg(int i, LiveSet *live) {
  ...
  if ((NULL != v) && _reg[i]->dirty && live->contains(v)) {
    ...
    oss << "spill " << v << " from " << _reg[i]->name << " to (" << base->name
        << (v->offset < 0 ? "" : "+") << v->offset << ")";
