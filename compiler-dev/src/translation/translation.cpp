...
void Translation::visit(ast::Program *p) {
  // 访问全局变量和函数
  for (auto it = p->func_and_globals->begin(); it != p->func_and_globals->end();
       ++it)
    (*it)->accept(this);
}
...
void Translation::visit(ast::FuncDefn *f) {
  ...
  // 传入变量
  for (ast::VarList::iterator it = f->formals->begin(); it != f->formals->end();
       ++it) {
 ...
 */
void Translation::visit(ast::ContinueStmt *s) {
...
void Translation::visit(ast::VarDecl *decl) {
  ...
  if (decl->ATTR(sym)->isGlobalVar()) {
    ...
    if (decl->type->ATTR(type)->isArrayType()) {
      ...
      decl->ATTR(sym)->attachTemp(
          tr->allocNewTempI4(decl->type->ATTR(type)->getSize()));
    } else {
